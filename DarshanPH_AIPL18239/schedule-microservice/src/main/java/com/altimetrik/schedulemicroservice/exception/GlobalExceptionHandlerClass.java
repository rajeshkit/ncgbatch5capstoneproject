...
public class GlobalExceptionHandlerClass {
    ...
    public ResponseEntity<?> handlerTrainIdNotExists(ScheduleIdNotExistsException ex, HttpServletRequest request){
        ApiError apiError=ApiError.builder().apiErrorDate(new Date()).path(request.getRequestURI()).message(Arrays.asList(ex.getLocalizedMessage())).status(HttpStatus.NOT_FOUND.toString()).build();
        return new ResponseEntity<>(apiError, HttpStatus.NOT_FOUND);
    }
    @ExceptionHandler({TrainNotExistsException.class, RouteNotExistsException.class})
    ...
    public ResponseEntity<?> handlerMethodInvalidArgBindingException(MethodArgumentNotValidException ex,HttpServletRequest request){
        ...
        List<String> listOfError=dlist.stream()
